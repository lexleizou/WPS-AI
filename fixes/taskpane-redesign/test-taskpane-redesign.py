...
check("document map summary preview gate", all(token in js for token in ["installDocumentPreviewGate", "installDocumentPreviewToolGate", "wps_get_document_map", "阶段二：只读修改预览", "确认按预览修改", "PREVIEW_GATE_REQUIRED", "READ_ONLY_SCAN_NO_NAVIGATION", "reveal_location", "autoPreviewPending", "clickChatStopSilently(\"preview-map-boundary\")", "禁止分段生成完整文稿"]) and ".lingxi-write-preview-gate" in css)
check("internal stage stop is silent", all(token in js for token in ["isSyntheticChatStop", "clickChatStopSilently", "lingxiSyntheticStop", "removeSyntheticNotice", "（已停止）"]) and js.count("if (isSyntheticChatStop(") >= 2)
