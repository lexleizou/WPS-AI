...
check("chat paragraph anchor navigation", all(token in js for token in ["extractParagraphAnchors", "revealChatAnchor", "installChatAnchorNavigation", "ScrollIntoView", "lingxi-chat-anchor-link", "MutationObserver", "跳转到 Word"]) and ".Select" not in anchor_navigation and all(token in css for token in [".lingxi-chat-anchor-link", ".lingxi-chat-anchor-feedback"]))
reference_bridge = js.split("let pendingAgentReferenceRequest", 1)[1].split("function installChatCopyShortcut", 1)[0]
check("agent reference one-turn request bridge", all(token in js for token in ["pendingAgentReferenceRequest", "expiresAt: Date.now() + (2 * 60 * 1000)", "installAgentReferenceRequestBridge", "role: \"system\"", "[Agent references — use as supplementary context only]", "finally", "clearPendingAgentReferences", "injectionStarted"]) and "state.consume" not in reference_bridge and "chatInput.value = pending" not in js and "chatHistory" not in reference_bridge)
