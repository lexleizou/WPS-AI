...
check("Codex concise current-provider label", "Codex · 直连" in js and "ChatGPT OAuth" in js and "真实 modelId 仍来自隐藏 select" in js)
check("Codex picker group stays identifiable", all(token in js for token in ["syncCodexModelPickerLabels", "installCodexModelPickerLabels", "Codex · 官方直连"]) and "position: sticky" in css and ".lg-composer-model .model-group-head" in css)
